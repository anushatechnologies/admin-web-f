...
export const storeTypeApi = baseApiWithAuth.injectEndpoints({
    endpoints: (builder) => ({
        getStoreTypes: builder.query<{ content: Store[]; totalPages: number; totalElements: number }, { name?: string; page?: number; size?: number }>({
        ...
        suggestStoreTypes: builder.query<Store[], string>({
            query: (query) => `/stores/suggest?q=${encodeURIComponent(query)}`,
            providesTags: ['Stores'],
        }),
    }),
});

export const { useGetStoreTypesQuery, useSuggestStoreTypesQuery } = storeTypeApi;

// --- MANUAL FETCH (for Store_type.tsx) ---
